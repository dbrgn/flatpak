 ...
 *
 * When this is set to %TRUE, Flatpak will automatically install the debug
 * info for each app currently being installed or updated. Does nothing if an
 * uninstall is taking place.
 *
 * Since: 1.9.4
 */
void
          ...
          if (!download)
            {
              ...
              if (g_str_has_suffix (id, ".Debug") && priv->auto_install_debug)
                download = TRUE;
            }
